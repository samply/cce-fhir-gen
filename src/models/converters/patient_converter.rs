...
impl CategoryConverter for Patient {
    ...
    fn get_category() -> Category {
        ...
        let vital_status =
            SingleSelectCategory::new("75186-7", "Vital Status", get_vital_status_url().value.unwrap_or_default().as_str(), VitalStatus::get_criteria());

        let child_categories = vec![Category::SingleSelect(gender), Category::SingleSelect(vital_status)];
